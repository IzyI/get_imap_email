...
#
#
# Отличие от другого скрипта в том что сдесь реализованна многопоточность
# c простой очередью
#
#
...
import sys, imaplib, email, pprint, os, re, traceback, datetime, threading
...
class Downloader(threading.Thread):
    ...
    def run(self):
        """Запуск потока"""
        while True:
            # Получаем массив с логином и паролем из очереди
            s = self.queue.get()

            # Скачиваем файл
            self.download_file(s[0], s[1])

            # Отправляем сигнал о том, что задача завершена
            self.queue.task_done()

    def download_file(self, login, pawssword):
        """Скачиваем файл"""
...
def main(result_email):
    '''

    :param result_email: получаем коректный список emailов
    :return:  вобщем то тут происходит вся работа, создаем и пишем в файлы

    '''
    queue = Queue()
    for i in range(len(result_email.keys())):

        t = Downloader(queue)
        t.setDaemon(True)
        t.start()

    for s in result_email.items():
        queue.put(s)
    # Ждем конца очереди
    queue.join()
